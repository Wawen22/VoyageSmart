...
import { Building2Icon, CrownIcon, SparklesIcon, TrendingUpIcon } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { cn } from '@/lib/utils';

interface AccommodationCounterWidgetProps {
  count: number;
  className?: string;
  compact?: boolean; // For mobile view
}

export default function AccommodationCounterWidget({ count, className, compact = false }: AccommodationCounterWidgetProps) {
  const { subscription } = useSubscription();

  // Don't show for premium/AI users (they have unlimited accommodations)
    ...
    <div className={cn(
      ...
      <div className="relative flex items-center gap-2 flex-1">
        <div className="flex items-center gap-2">
          ...
          {!compact && (
            <span className="text-xs font-medium text-muted-foreground">accommodations</span>
          )}
        </div>
